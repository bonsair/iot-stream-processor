...
def generate(host, port, topic, sensors, message, interval,iThread):
    ...
    interval_secs = interval/ 500.0#Cambiado estaba a 1000
    loop = 0
    #Start timer
    ...
    while loop<(message):
        ...
        sensor['messageId'] = sensor['id']+str(loop)
        sensor['timestamp'] = int(time.time())
        sensor['metrics'] = {}

        #data with correct and incorrect values ​​is
        #generated for processing
        if loop%100==0:
            sensor['metrics'] = {}
        elif loop%10==0:
            sensor['metrics']['temperature'] = random.randint(50,80)
            sensor['metrics']['humidity'] = random.randint(100,130)
        else:
            sensor['metrics']['temperature'] = random.randint(10,50)
            sensor['metrics']['humidity'] = random.randint(50,80)
